...
template <typename Address> struct jmp_to {};

template <> struct jmp_to<uint32_t>
{
...
template <> struct jmp_to<uint64_t>
{
...
template <typename Address> struct call {};

template <> struct call<uint32_t>
{
    typedef uint32_t address_t;
...
template <typename Address> struct shellcode_thunk {};

template <> struct shellcode_thunk<uint32_t>
{
    typedef uint32_t address_t;
...
template <> struct shellcode_thunk<uint64_t>
{
    typedef uint64_t address_t;
...
# define CTX_EAX ctx.Eax
    ...
    const size_t bootstrapper_dll_len_size = bootstrapper_dll.length() * sizeof(wchar_t) +
        sizeof(wchar_t),
    ...
    // write
    if (!WriteProcessMemory(hprocess, blob, bootstrapper_dll.c_str(), bootstrapper_dll_len_size,
            NULL)) {
        return GetLastError();
    }

    if (!WriteProcessMemory(hprocess, (char *)blob + bootstrapper_dll_len_size,
            audiorouter_dll.c_str(),
    ...
    if (!WriteProcessMemory(hprocess, (char *)blob + bootstrapper_dll_len_size +
            audiorouter_dll_len_size + sizeof(shellcode_thunk_t), shellcode, sizeof(shellcode),
            NULL))
    {
        return GetLastError();
    }
