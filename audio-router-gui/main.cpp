...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    /*global_routing_params* params = new global_routing_params;
       ...
       MessageBox(NULL, (wchar_t*)((global_routing_params*)serialized_params->next_global_ptr)->local.device_id_ptr,
          NULL, 0);
    ...
    if (hr != S_OK) {
        CloseHandle(audio_router_mutex);
        MessageBox(NULL,
            L"COM could not be initialized. Audio Router will close.",
            NULL,
            MB_ICONERROR);
        return 0;
    }
